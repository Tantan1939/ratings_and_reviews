...
namespace CustomerDataRatingsAndReviewsManagementSystem
{
    public class NotificationComponent
    {
        ...
        public void RegisterNotification(DateTime currentTime)
        {
            ...
            string sqlCommand = @"SELECT [notificationID],[Username] from [dbo].[notificationTable] where [AddedOn] > @AddedOn";
        ...
        public List<notificationTable> GetData(DateTime afterDate)
        {
            using (SignalRDBentities dc = new SignalRDBentities())
            {
                return dc.notificationTables.Where(a => a.AddedOn > afterDate).OrderByDescending(a => a.AddedOn).ToList();
            }
        }
    }
}